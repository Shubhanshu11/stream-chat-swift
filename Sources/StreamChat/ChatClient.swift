...
public class ChatClient {
    ...
    private var isFirstConnection: Bool {
        switch tokenHandler.connectionProvider {
        case .noCurrentUser, .notInitiated:
            return true
        case .initiated:
            return false
        }
    ...
    public func disconnect() {
        ...
        tokenHandler.connectionProvider = currentUserId.map(UserConnectionProvider.notInitiated) ?? .noCurrentUser
    }
...
extension ChatClient: ConnectionStateDelegate {
    func webSocketClient(_ client: WebSocketClient, didUpdateConnectionState state: WebSocketConnectionState) {
        ...
        case let .disconnected(source):
            ...
            if let error = source.serverError, error.isInvalidTokenError && !isFirstConnection {
